...
fun shareImage(context: Context, imagePath: String, packageId: String) =
    if (context.packageManager.getLaunchIntentForPackage(packageId) == null) {
        ...
        context.composeImageUri(imagePath).let { context.startIntent(packageId, it) }
    }
...
private fun Context.composeImageUri(imagePath: String): Uri = "${applicationContext.packageName}.provider"
